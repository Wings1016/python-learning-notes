...
# 1.创建连接池，便于所有HTTP连接直接获取数据库连接。使用连接池的好处是不必频繁地打开和关闭数据库连接，而是能复用就尽量复用。
...
async def select(sql, args, size=None):
    ...
    with (await __pool) as conn:
        ...
        if size:    # 如果传入size参数，就通过fetchmany()获取最多指定数量的记录
            rs = await cur.fetchmany(size)
        else:       # 否则，通过fetchall()获取所有记录。
            rs = await cur.fetchall()
        await cur.close()
        logging.info('rows returned: %s' % len(rs))
        return rs

# 3.execute函数，执行insert、delete、update
